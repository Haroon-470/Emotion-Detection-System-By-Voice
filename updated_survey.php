...
<body>
    ...
    <script>
        ...
        // Modified analyzeEmotion function to store results in database
async function analyzeEmotion() {
    ...
    try {
        ...
        // Save dominant emotion to database
        const q_id = currentQuestion + 1;
        const dominant_emotion = result.dominant_emotion;
        
        // Call new endpoint to store emotion data
        await storeEmotionData(q_id, dominant_emotion);
        ...
        // New function to store emotion data in the database
async function storeEmotionData(q_id, dominant_emotion) {
    try {
        const formData = new FormData();
        formData.append('q_id', q_id);
        formData.append('dominant_emotion', dominant_emotion);
        
        const response = await fetch('store_emotion.php', {
            method: 'POST',
            body: formData
        });
        ...
        if (result.success) {
            console.log('Emotion data stored successfully');
        } else {
            console.error('Failed to store emotion data:', result.message);
        }
    } catch (error) {
        ...
        function getEmotionColorClass(emotion) {
            ...
            if (emotionLower.includes('fear')) return 'fear';
            return '';
        }
    </script>
</body>
